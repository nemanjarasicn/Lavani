...
export default  class Odmor_unos_edit extends Component{
	constructor(props){
		super(props)
	...
	render(){
		...
		return (
	  		<View style={this.H.ObjAss({}, style.column, style.flex_1)}>
	  			...
	  			<TouchableOpacity onPress={()=>{params.navigation.navigate('Novi_odmor', {
	    			...
	    			refresh: (data)=>{this.setState({data:data})}
		  			...
		  			<View style={this.H.ObjAss({height:50, backgroundColor:'#ddd'}, style.center, style.flex_0, style.mar_15)}>
		  				<Text style={this.H.ObjAss({fontSize:16}, style.txt_c)}>{txt.odmori_novi} +</Text>
		  			</View>
		  		</TouchableOpacity>
	  			<View style={{flex:1}}>
					<SearchBar        
					...
					<View style={this.H.ObjAss({height:40, borderBottomWidth: 1}, style.row, style.flex_0, style.center)}>
						<View style={this.H.ObjAss({width: 60}, style.flex_0, style.center)}>
							<Text style={style.txt_c}>{txt.godina}</Text>
						</View>
						<View style={this.H.ObjAss({}, style.column, style.flex_1, style.center)}>
							<Text style={style.txt_c}>{txt.deo_godisnjeg}</Text>
							<Text style={style.txt_c}>{txt.od} - {txt.do}</Text>
						</View>
						<View style={this.H.ObjAss({width: 60}, style.flex_0, style.center)}>
							<Text style={style.txt_c}>{txt.trajanje}</Text>
						</View>
					</View>
					<FlatList          
						scrollEnabled={true}
					    ...
					    renderItem={({ item }) => ( 
					    	<TouchableOpacity onPress={()=>{						
					    		this.navigation.navigate('Novi_odmor', {
					    			idOdmor: item[0],
					    			godina: item[1],
					    			idRadnik: this.idRadnik,
					    			radnik_ime: this.radnik_ime,
