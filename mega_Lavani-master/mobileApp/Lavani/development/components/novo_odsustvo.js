...
export default  class Novo_odsustvo extends Component{
	constructor(props){
	    ...
	    this.idOdsustvo = (props.params.route.params.idOdsustvo != undefined)? props.params.route.params.idOdsustvo : '' ;
		...
		if(this.idOdsustvo != ''){
			this.H.callFetch({
				...
				func: (res)=>{
					...
					this.props.params.set.sifVrstaOdsustva({value:data[2], label: data[7]})
	...
	izracunajBrDana = () => {
		...
		if(datum_od > datum_do) Alert.alert(this.txt.upozorenje, this.txt.datum_manji)
			...
			this.H.callFetch({
				...
				func: (data)=>{this.props.params.set.brojDana(data)}
			})
		}		
	}
		  			...
		  			<ScrollView >
		  				...
		  				<View style={this.H.ObjAss({}, style.mar_15, style.column)}>
			  				...
			  				<View style={this.H.ObjAss({height:50, borderWidth:1}, style.txt_c)}>
				  				<RNPickerSelect
						            onValueChange={(value, index) => {params.set.sifVrstaOdsustva({value: value, label: this.sifVrstaOdsustva[index-1].label})}}
		  				...
		  				<TouchableOpacity onPress={()=>{this.izracunajBrDana()}
		  				}>
			  			...
			  			<View style={this.H.ObjAss({}, style.mar_15, style.column)}>
			  				...
			  				<TextInput style={this.H.ObjAss({height:50, borderWidth:1}, style.txt_c)} placeholder={txt.broj_dana}
			  					onChangeText={(value)=>{params.set.brojDana(value)}}
		  				...
		  				<View style={this.H.ObjAss({}, style.mar_15, style.column)}>
			  				...
			  				<TextInput style={this.H.ObjAss({height:180, borderWidth:1}, style.padd_15)} placeholder={txt.napomena}
			  					onChangeText={(value)=>{params.set.napomena(value)}}
								...
								if(res){
									this.H.callFetch({
										...
										func:(res)=>{
											...
											params.navigation.navigate('Odsustvo_unos_edit', {entitet: this.entitet})
										}
									})	
