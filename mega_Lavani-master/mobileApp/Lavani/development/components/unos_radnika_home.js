...
export default  class Unos_radnika_home extends Component{
	constructor(props) {
	    super(props);
	    this.state 	 	= {data: []};
	    this._arr  	 	= []
		...
		return (
	  		<View style={this.H.ObjAss({}, style.column, style.flex_1)}>
	  			<View style={this.H.ObjAss({height:60}, style.center, style.flex_0)}>
	  				<Text style={this.H.ObjAss({fontSize:18}, style.txt_c)}>{txt.radnici}</Text>
	  			</View>
	  			<TouchableOpacity onPress={()=>{
	  					params.navigation.navigate('Edit_unos_radnika', {
		  			...
		  			<View style={this.H.ObjAss({height:50, backgroundColor:'#ddd'}, style.center, style.flex_0, style.mar_15)}>
		  				<Text style={this.H.ObjAss({fontSize:16}, style.txt_c)}>{txt.novi_radnik} +</Text>
		  			</View>
		  		</TouchableOpacity>
	  			<View style={{flex:1}}>
					<SearchBar        
					...
					<FlatList          
					    ...
					    renderItem={({ item }) => ( 
					    	<TouchableOpacity onPress={()=>{
					    		this.navigation.navigate('Edit_unos_radnika', {
					    			idRadnik: item[0],
